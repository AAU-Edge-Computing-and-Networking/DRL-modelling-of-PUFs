# -*- coding: utf-8 -*-
"""
Created on Tue Aug  1 12:38:38 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an Arbiter or XOR PUF using
replicated Double DQN. Requires ChainerRL v0.8.0.
"""
...
def main():
    ...
    parser.add_argument("--replicants", type=int, default=2,
                        help="Number of agents to train the same model.")
    ...
    for i in range(args.replicants):
        ...
        agent.append(
            agents.DoubleDQN(
                q_func, optimizer, r_buffer[i], args.gamma, explorer,
    ...
    for i in range(args.train_data):
        ...
        while(t < args.max_episode_len and not done):
            ...
            for a in agent:
                action = a.act_and_train(obs, reward)
        ...
        for a in agent:
            a.stop_episode_and_train(obs, reward, done)
    ...
    data = pd.DataFrame({"seed": [args.seed],
                         ...
                         "accuracy": [accuracy]})
    filepath = Path(args.outdir + "out_DQN_replicant_" + str(args.k) + "XOR.csv")
    if(filepath.is_file()):
