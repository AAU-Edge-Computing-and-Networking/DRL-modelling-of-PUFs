# -*- coding: utf-8 -*-
"""
Created on Wed Sep 27 14:26:17 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an XOR Arbiter PUF using MLP.
"""
...
def main():
    ...
    if(args.k <= 3):
        ...
        # As defined in the literature: [2^(k-1), 2^k, 2^(k-1)]
    ...
    data = pd.DataFrame({"seed": [args.seed],
                         ...
                         "accuracy": [accuracy]})
    filepath = Path(args.outdir + "out_MLP_" + str(args.k) + "XOR.csv")
    if(filepath.is_file()):
