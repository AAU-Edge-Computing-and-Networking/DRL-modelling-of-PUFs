# -*- coding: utf-8 -*-
"""
Created on Tue Aug  1 13:01:25 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an Arbiter or XOR PUF using multiple
Double DQN agents with majority voting. Requires ChainerRL v0.8.0.
"""
...
def main():
    ...
    parser.add_argument("--voters", type=int, default=3,
                        help="Number of agents to vote on a prediction.")
    args = parser.parse_args()
    
    # Check if number of voters in odd
    assert args.voters % 2 != 0, "Number of voting agents should be odd!"
    ...
    for i in range(args.voters):
        ...
        # Optimizer
        optimizer.append(optimizers.Adam(eps=1e-3))
        optimizer[i].setup(q_func[i])
    
        # Replay buffer
        r_buffer.append(
            replay_buffer.ReplayBuffer(capacity=args.replay_buffer_size))
    
        # Agent
        agent.append(
            agents.DoubleDQN(
                q_func[i], optimizer[i], r_buffer[i], args.gamma, explorer[i],
    ...
    for a in range(len(agent)):
        ...
        for i in range(args.train_data):
            ...
            while(t < args.max_episode_len and not done):
                ...
                action = agent[a].act_and_train(obs, reward)
                
                # Check result
                _ , reward, done = env.step(action)
            
            # Train on episode end
            agent[a].stop_episode_and_train(obs, reward, done)
    ...
    for i in range(args.train_data, args.train_data + args.test_data):
        ...
        action = []
        for a in agent:
            action.append(a.act(obs))
        
        # Check prediction
        _ , reward, _ = env.step(round(sum(action)/len(action)))
        total_reward += reward
    ...
    data = pd.DataFrame({"seed": [args.seed],
                         ...
                         "voters": [args.voters],
                         "accuracy": [accuracy]})
    filepath = Path(args.outdir + "out_DQN_vote_" + str(args.k) + "XOR.csv")
    if(filepath.is_file()):
